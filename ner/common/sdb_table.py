...
from sdb_tree_color_scheme import *
...
class SDBComponentBranch(BranchNode):
    ...
    def setup_component_features(self):
        ...
        if self.component.is_device():
            vendor_id = "0x%016X" % self.component.get_vendor_id_as_int()
            device_id = "0x%08X" % self.component.get_device_id_as_int()

            abi_class = "%d" % self.component.get_abi_class_as_int()
            abi_major = "0x%08X" % self.component.get_abi_version_major_as_int()
            abi_minor = "0x%08X" % self.component.get_abi_version_minor_as_int()
            device_version = self.component.get_version_as_int()
            year, month, day = self.component.get_date_as_int()
            date = "%d/%d/%d" % (year, month, day)
            
            child = SDBEntryNode(self, "Vendor ID", vendor_id, self.depth)
            self.insertChild(child)
            child = SDBEntryNode(self, "Device ID", device_id, self.depth)
            self.insertChild(child)
            child = SDBEntryNode(self, "ABI Class", abi_class, self.depth)
            self.insertChild(child)
            child = SDBEntryNode(self, "ABI Major", abi_major, self.depth)
            self.insertChild(child)
            child = SDBEntryNode(self, "ABI Minor", abi_minor, self.depth)
            self.insertChild(child)
            child = SDBEntryNode(self, "Version", device_version, self.depth)
            self.insertChild(child)
